...
-- Índices panel maestro
CREATE INDEX idx_panel_cliente_id ON panel_maestro_metrica(cliente_id);

-- 10. TABLA AUDITORIA CAMBIOS
...
-- 4. Trigger para actualizar fecha de actualización
...
-- Aplicar a todas las tablas que tienen fecha_actualizacion
...
-- Vista de resumen de cliente para dashboard
...
-- Función para obtener el resumen de un cliente
CREATE OR REPLACE FUNCTION obtener_resumen_cliente(p_cliente_id UUID)
RETURNS TABLE (
  total_cursos BIGINT,
  total_participantes BIGINT,
  total_acreditados BIGINT,
...
BEGIN
  ...
  LEFT JOIN curso_participante cp ON cur.id = cp.curso_id
  LEFT JOIN pagos p ON cp.id = p.curso_participante_id
  WHERE c.id = p_cliente_id AND c.estado = 'activo'
  GROUP BY c.id;
END;
$$ LANGUAGE plpgsql;

-- Función para validar certificado público
CREATE OR REPLACE FUNCTION validar_certificado_publico(p_codigo_validacion VARCHAR)
RETURNS TABLE (
  valido BOOLEAN,
  participante_nombre VARCHAR,
...
BEGIN
  ...
  SELECT 
    ...
    cp.fecha_expiracion as fecha_expiracion,
    'Certificado válido' as mensaje
  FROM curso_participante cp
  JOIN participantes p ON cp.participante_id = p.id
  JOIN cursos c ON cp.curso_id = c.id
  WHERE cp.codigo_validacion = upper(p_codigo_validacion)
    AND cp.estado_acreditacion = true
    AND cp.fecha_expiracion >= CURRENT_DATE;
  
  IF NOT FOUND THEN
    RETURN QUERY
    SELECT false, '', '', null, null, 'Certificado no válido o expirado';
  END IF;
END;
$$ LANGUAGE plpgsql;
