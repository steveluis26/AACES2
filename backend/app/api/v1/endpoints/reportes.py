...
async def constancias_por_periodo(
    periodo: str = Query("month", regex="^(month|quarter|year)$"),
    ...
    sql = text(f"""
        SELECT
            date_trunc(:trunc, d.fecha_emision) AS periodo,
            COUNT(*) AS total
...
async def tiempo_promedio_emision(
    ...
    sql = text(f"""
        SELECT AVG(
            EXTRACT(EPOCH FROM (d.fecha_emision - cp.fecha_emision_certificado))
            / 86400.0
        ) AS promedio_dias
        FROM aaces.documentos_emitidos d
    ...
    promedio = float(row[0]) if row and row[0] else None
    return {"promedio_dias": round(promedio, 2) if promedio is not None else None}
